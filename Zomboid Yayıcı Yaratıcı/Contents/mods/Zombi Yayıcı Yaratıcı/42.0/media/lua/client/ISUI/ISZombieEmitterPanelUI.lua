...
function ISZombieEmitterPanelUI:createChildren()
    ...
    local labelX = 150 - (getTextManager():MeasureStringX(UIFont.Large, "Zombi Çıkartıcı Bölgeler") / 2)
    self.zombieEmitterLbl = ISLabel:new(labelX, y, 20, "Zombi Çıkartıcı Bölgeler", 1, 1, 1, 1, UIFont.Large, true)
    ...
    for i, zoneType in ipairs(zoneTypes) do
        ...
        button.backgroundColorMouseOver = zoneType.backgroundColorMouseOver or { r = 0.2, g = 0.2, b = 0.6, a = 1 }; -- Fare üzerine gelince daha koyu mavi
...
function ISZombieEmitterPanelUI:onClick(button)
    ...
    elseif button.internal == "DELETE_ZONE" then
        ...
        local text = "\"" .. self.selectedZone.zoneName .. "\" bölgesini silmek istediğinize emin misiniz? Bu işlem geri alınamaz!"
...
function ISZombieEmitterPanelUI:prerender()
    ...
    self:drawText("Zombi Çıkartıcı",
        self.width / 2 - (getTextManager():MeasureStringX(UIFont.Title, "Zombi Çıkartıcı") / 2), 20, 1, 1, 1, 1,
        UIFont.Title)
    ...
    if not self.selectedZone then
        local message = "Hiçbir bölge seçilmedi."
        ...
        local spawnIntervalStr = string.format("%d %s(s)", zoneDetails.spawnInterval,
        ...
        self:drawText("Toplam Çıkarılan Zombiler", x + 10, detailsY, 1, 1, 1, 1, UIFont.Medium)
        detailsY = detailsY + 17
    ...
    if not self.selectedZone then
        ...
        self.teleportBtn.tooltip = "Seçilen bölgeye ışınlan"

        self.editZoneBtn:setEnable(true)
        self.editZoneBtn.tooltip = "Seçilen bölgeyi düzenle"

        self.removeZoneBtn:setEnable(true)
        self.removeZoneBtn.tooltip = "Seçilen bölgeyi sil"
    end
end
