...
function ISZombieCullerCreateZoneUI:createChildren()
    ...
    -- Bölge Adı Giriş Kutusu
    self.zoneName = ISTextEntryBox:new("İsimsiz Bölge", self.zoneNameLbl.x, self.zoneNameLbl:getBottom() + 5, 120, 20);
    ...
    self.maxZombiesLbl = ISLabel:new(x, y, 10, "Maks. Zombi", 1, 1, 1, 1, UIFont.Medium,
        true);
    self:addChild(self.maxZombiesLbl);

    -- Maksimum Zombi Sayısı Giriş Kutusu
...
---Bölge üzerine yazma onayını işler
---@param this ISZombieCullerCreateZoneUI
...
function ISZombieCullerCreateZoneUI:onClick(button)
    ...
    elseif button.internal == "CREATE_ZONE" then
        ...
        if ZombieEmitterUtils.doesCullerZoneExist(zoneName) then
            ...
            local text =
            "Bu isimde bir temizleyici bölge zaten var. Devam ederseniz üzerine yazılacaktır. Devam etmek istediğinizden emin misiniz?"
...
function ISZombieCullerCreateZoneUI:prerender()
    ...
    if not self.isZoneSelected then
        ...
        self.createZoneBtn.tooltip = "<RED> Önce bölgeyi seçmelisiniz"
    else
...
---Fare panel dışında hareket ederse işler
---@param dx number
---@param dy number
...
---Fare panel dışında bırakılırsa işler
---@param x number
---@param y number
...
---Fare panel dışında tıklanırsa işler
---@param x number
---@param y number
...
---Yeni bir ISZombieCullerCreateZoneUI örneği oluşturur
---@param x number
---@param y number
---@param character IsoPlayer
---@return ISZombieCullerCreateZoneUI
