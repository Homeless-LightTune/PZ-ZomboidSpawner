...
local function onFillWorldObjectContextMenu(player, context, worldobjects)
    if isAdmin() or isDebugEnabled() or getSpecificPlayer(player):isAccessLevel('admin') then
        ...
        -- Zombi Emitter seçeneğini ekle
        local emitterOption = context:addOption("Zombi Emitter")
        ...
        local emitterZones = subMenu:addOption("Emitter Bölgeleri", playerObj, onOpenZombieEmitterUI)
...
-- Dünya nesnesi bağlam menüsüne seçenekleri ekle
