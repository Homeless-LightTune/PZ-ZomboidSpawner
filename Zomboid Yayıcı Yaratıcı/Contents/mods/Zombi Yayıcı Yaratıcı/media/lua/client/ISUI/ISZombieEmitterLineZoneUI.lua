...
function ISZombieEmitterLineZoneUI:createChildren()
    ...
    self.spawnInterval.tooltip = "Zombilerin gerçek zamanlı olarak çıkış aralığı"
    ...
    self.zombiesNbr.tooltip = "Her seferinde çıkacak zombi sayısı"
    ...
    self.maxZombies.tooltip = "Bölgenin zombi çıkarmayı durduracağı maksimum zombi sayısı. Sonsuz için -1 olarak ayarlayın"
...
function ISZombieEmitterLineZoneUI:onCreateZone()
    ...
    if not ZombieEmitterUtils.doesZoneExist(zoneName) then
        ...
        local text =
        "Bu isimde bir bölge zaten var. Devam etmek mevcut bölgeyi üzerine yazacaktır. Devam etmek istediğinize emin misiniz?"
...
function ISZombieEmitterLineZoneUI:prerender()
    ...
    if self.BselectX == 0 and self.BselectY == 0 and self.BselectZ == 0 then
        ...
        self.add.tooltip = "<KIRMIZI> Lütfen bir bitiş noktası seçin"
    else
